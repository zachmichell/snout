// Deposit-prepayment series (PR-5): list the operator's QBO Accounts
// Receivable accounts for the Customer-Deposit "apply to invoice" picker.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.4";
import { corsHeaders } from "../_shared/cors.ts";
import { requireOrgAdmin } from "../_shared/auth.ts";
import { getTokenContext, listAccountsReceivableAccounts } from "../_shared/quickbooks.ts";
...
Deno.serve(async (req) => {
  ...
  const res = await listAccountsReceivableAccounts(tokens);
