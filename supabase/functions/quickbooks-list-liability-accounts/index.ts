// 6.6b/c: List the operator's QBO Liability accounts (Other Current
// Liability + Long Term Liability) for the Tips Payable and Deferred
// Revenue dropdowns.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.4";
import { corsHeaders } from "../_shared/cors.ts";
import { requireOrgAdmin } from "../_shared/auth.ts";
import { getTokenContext, listLiabilityAccounts } from "../_shared/quickbooks.ts";
...
Deno.serve(async (req) => {
  ...
  const res = await listLiabilityAccounts(tokens);
