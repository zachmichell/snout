// 6.6a: List the operator's QBO Expense + COGS accounts so the
// settings tab can render a "Fee account" dropdown for processor
// payouts. Two-pass under the hood since QBO query language doesn't
// accept parenthesized OR clauses.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.4";
import { corsHeaders } from "../_shared/cors.ts";
import { requireOrgAdmin } from "../_shared/auth.ts";
import { getTokenContext, listExpenseAccounts } from "../_shared/quickbooks.ts";
...
Deno.serve(async (req) => {
  ...
  const res = await listExpenseAccounts(tokens);
